...
DIVERGENCE = Template(
    ...
    notes=[
        "「回落」是相对概念，必须有参照系。本模板的参照系是标的自上市以来（最多回溯 10 年）"
        "的自身历史分布，而非某个绝对数值。",
    ...
    sub_questions=[
        _sq(
            "SQ-01",
            "标的当前估值处于自身历史的什么位置？是否确实已回落？",
            "重建 PE 序列的当前分位",
            "扶摇 历史K线（前复权）+ 合并利润表 → 本地重建 PE 序列；与扶摇估值快照 pe_ttm 对拍校准",
            "自序列起始日（最多回溯 10 年）至最新交易日",
            "计算重建 PE 当前值在自身历史序列中的分位；分位越低代表估值越靠近历史低位。"
            "同时必须报告重建值与官方 pe_ttm 的相对偏差；偏差超容差则本条结论降级为低置信度",
